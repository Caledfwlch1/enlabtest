...
func (p *postgres) RollBackLastN(ctx context.Context, task *types.RollBackTask) error {
	dops, err := p.GetLastRecords(ctx, task.RecNumb)
	if err != nil {
		return err
	}
...
func (p *postgres) GetLastRecords(ctx context.Context, n int) ([]*types.DataOperation, error) {
	query := `SELECT transaction_id, state, amount, user_id FROM transaction 
ORDER BY timestamp DESC LIMIT $1;`

	rows, err := p.db.QueryContext(ctx, query, n*2)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		out = append(out, &types.DataOperation{
